# AI-FanYi 工单列表

本文档列出所有工单及其状态。

## 工单状态

- `todo`: 待处理
- `doing`: 进行中
- `done`: 已完成
- `blocked`: 已阻塞

## 工单列表

### Layer 0 Orchestrator

| ID | 工单名称 | 状态 | 优先级 | 依赖 |
|----|---------|------|--------|------|
| 001 | Layer 0 数据库模型和迁移 | todo | 高 | - |
| 002 | Artifact Registry 实现 | todo | 高 | 001 |
| 003 | REST API - 项目和作业管理 | todo | 高 | 001, 002 |
| 004 | Worker 管理器实现 | todo | 高 | 001, 003 |
| 005 | 调度器核心实现 | todo | 高 | 001, 002, 004 |
| 015 | WebSocket 实时通信 | todo | 中 | 003, 013 |

### M04 - Character Database

| ID | 工单名称 | 状态 | 优先级 | 依赖 |
|----|---------|------|--------|------|
| 006 | M04 人物数据库核心实现 | todo | 高 | 001, 002 |
| 007 | M04 人物关系构建 | todo | 中 | 006 |

### M05 - Audio & Scene Analysis

| ID | 工单名称 | 状态 | 优先级 | 依赖 |
|----|---------|------|--------|------|
| 008 | M05 音频分析核心实现 | todo | 高 | 001, 002 |

### M06 - Speaker Mapping

| ID | 工单名称 | 状态 | 优先级 | 依赖 |
|----|---------|------|--------|------|
| 009 | M06 说话人到人物映射 | todo | 高 | 001, 002, 006, 008 |

### M07 - Dialogue Intelligence & M08 - Prosody Planning

| ID | 工单名称 | 状态 | 优先级 | 依赖 |
|----|---------|------|--------|------|
| 010 | M07 对白智能处理 + M08 韵律规划 | todo | 高 | 001, 002, 006, 008, 009 |

### M09 - Voice Synthesis

| ID | 工单名称 | 状态 | 优先级 | 依赖 |
|----|---------|------|--------|------|
| 011 | M09 语音合成核心实现 | todo | 高 | 001, 002, 010 |

### M11 - Video Assembly

| ID | 工单名称 | 状态 | 优先级 | 依赖 |
|----|---------|------|--------|------|
| 012 | M11 视频组装与最终编码 | todo | 高 | 001, 002, 011 |

### Web Frontend

| ID | 工单名称 | 状态 | 优先级 | 依赖 |
|----|---------|------|--------|------|
| 013 | Web 前端框架搭建 | todo | 中 | 003 |
| 014 | Web 前端 - Dashboard 和 Projects 页面 | todo | 中 | 013 |

## 实施顺序

### Phase 1 (立即开始)
1. Ticket 001: Layer 0 数据库模型和迁移
2. Ticket 002: Artifact Registry 实现
3. Ticket 003: REST API - 项目和作业管理
4. Ticket 006: M04 人物数据库核心实现
5. Ticket 008: M05 音频分析核心实现

### Phase 2 (核心功能)
6. Ticket 004: Worker 管理器实现
7. Ticket 005: 调度器核心实现
8. Ticket 009: M06 说话人到人物映射
9. Ticket 010: M07 对白智能处理 + M08 韵律规划

### Phase 3 (语音和视频)
10. Ticket 011: M09 语音合成核心实现
11. Ticket 012: M11 视频组装与最终编码
12. Ticket 007: M04 人物关系构建

### Phase 4 (前端和实时)
13. Ticket 013: Web 前端框架搭建
14. Ticket 014: Web 前端 - Dashboard 和 Projects 页面
15. Ticket 015: WebSocket 实时通信

## 统计

- 总工单数: 15
- 待处理: 15
- 进行中: 0
- 已完成: 0
- 已阻塞: 0

## 备注

- 每完成 3 个工单，使用 code-review 技能检查一次
- 测试连续失败 3 次，标记工单为 blocked 并写明原因
- 每个 commit 后立即 git push
- 每轮结束确保 git 工作区干净
